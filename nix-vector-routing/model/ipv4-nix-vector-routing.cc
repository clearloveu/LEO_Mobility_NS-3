...
void
Ipv4NixVectorRouting::SetCacheDirty (bool isCacheDirty)
{
//  std::cout << "时间: "<< Simulator::Now () << " g_isCacheDirty = "<< g_isCacheDirty << std::endl;
  g_isCacheDirty = isCacheDirty;
}

void
Ipv4NixVectorRouting::UpdateNixVectorInCache(Ipv4Address destinationIp, const std::vector< Ptr<Node> > & parentVector, Ptr<Node> source, Ptr<Node> dest)
{
	  Ptr<Ipv4Route> rtentryInCache;
	  Ptr<NixVector> nixVectorInCache;
	  rtentryInCache = GetIpv4RouteInCache (destinationIp);
	  nixVectorInCache = GetNixVectorInCache (destinationIp);

      if (rtentryInCache)
        {
          m_ipv4RouteCache.erase (destinationIp);
        }
      if (nixVectorInCache)
        {
    	  m_nixCache.erase (destinationIp);
        }

      Ptr<NixVector> nixVector = Create<NixVector> ();
      if (BuildNixVector (parentVector, source->GetId (), dest->GetId (), nixVector))
        {
    	  // cache it
    	  m_nixCache.insert (NixMap_t::value_type (destinationIp, nixVector));
        }
      else
        {
...
std::vector< Ptr<Node> >
Ipv4NixVectorRouting::GetShortPath(Ptr<Node> source, Ptr<Node> dest){
	std::vector< Ptr<Node> > parentVector;
	BFS (NodeList::GetNNodes (), source, dest, parentVector, NULL);
	std::vector< Ptr<Node> > shortPath;
	Ptr<Node> temp = dest;
	while(temp != source){
		Ptr<Node> node = temp;
		shortPath.push_back(node);
		int id = temp->GetId();
		temp = parentVector[id];
	}
	Ptr<Node> node = source;
	shortPath.push_back(node);
	// 上面数组存的是des到src的路径，这里把路径反向
	std::vector< Ptr<Node> > shortPath2;
	for(int i = shortPath.size()-1;i>=0;i--){
		shortPath2.push_back(shortPath[i]);
	}
	return shortPath2;
}
...
{
//  std::cout << "时间: "<< Simulator::Now () << " routeOutput nodeIndex: " << m_node->GetId() << std::endl;
...
//  std::cout << "时间: "<< Simulator::Now () << " routeInput nodeIndex: " << m_node->GetId() << std::endl;
...
Ipv4NixVectorRouting::CheckCacheStateAndFlush (void) const
{
  if (g_isCacheDirty)
    {
//	  std::cout << "时间: "<< Simulator::Now () << "  CheckCacheStateAndFlush" << std::endl;
