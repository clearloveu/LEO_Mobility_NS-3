...
public:
  ...
  void SetCacheDirty(bool isCacheDirty);
  // 自己构造nixVector并放到cache中，add by zg
  void UpdateNixVectorInCache(Ipv4Address destinationIp, const std::vector< Ptr<Node> > & parentVector, Ptr<Node> source, Ptr<Node> dest);
  // 通过调用内部的bfs，返回从src到des的最短路径
  std::vector< Ptr<Node> > GetShortPath(Ptr<Node> source, Ptr<Node> dest);
